document.addEventListener('DOMContentLoaded', function() {
    ...
    function init() {
        ...
        const editingDesignId = localStorage.getItem('editingDesignId');
            if (editingDesignId) {
            // Загружаем состояние для редактирования
            const designLoaded = loadDesignState();
            if (designLoaded) {
                // Устанавливаем флаг, что это редактирование
    ...
    const currentPage = getCurrentPage();
    if (!currentPage) return imgContainer;
    
    const canvasRect = currentPage.canvas.getBoundingClientRect();
    
    // Рассчитываем максимальные размеры изображения, чтобы оно помещалось в канвас
    const maxWidth = canvasRect.width * 0.9; // 90% ширины канваса
    const maxHeight = canvasRect.height * 0.9; // 90% высоты канваса
    
    // Рассчитываем масштаб, чтобы изображение вписывалось в канвас
    const widthRatio = maxWidth / imgWidth;
    const heightRatio = maxHeight / imgHeight;
    const scaleFactor = Math.min(widthRatio, heightRatio, 1); // Не увеличиваем изображение больше оригинала
    ...
    // Позиционируем изображение по центру канваса
    imgContainer.style.left = `${(canvasRect.width - displayWidth) / 2}px`;
    imgContainer.style.top = `${(canvasRect.height - displayHeight) / 2}px`;
    ...
    function setupElementInteractions(element, type) {
        ...
        if (type === 'text') {
            ...
            // Обработчик клика - выделяем блок
        ...
        element.addEventListener('click', function(e) {
            if (e.target.classList.contains('resize-handle') || 
                e.target.classList.contains('image-menu-icon')) {
                return;
            }
            ...
            document.querySelectorAll('.image-menu-icon').forEach(icon => {
                icon.style.display = 'none';
            });
            
            const menuIcon = element.querySelector('.image-menu-icon');
        ...
        if (element.classList.contains('resizable')) {
            if (type === 'text') {
                ...
                makeResizable(element); // Стандартная реализация для изображений
            }
        }
    }
...
function makeResizable(element) {
    ...
    function addNewPage() {
        ...
        pageElement.style.transform = `scale(${state.currentZoom / 100})`;
    ...
    function updateActivePageIndicator() {
        state.pages.forEach(page => {
            page.element.classList.toggle('active', page.id === state.currentPage);
        });
    }
    ...
    // Применяем масштаб к страницам
    state.pages.forEach(page => {
        page.element.style.transform = `scale(${state.currentZoom / 100})`;
    });
    ...
    // Центрируем текущую страницу
    setTimeout(() => centerCurrentPage(true), 10);
}
    ...
    function centerCurrentPage(smooth = false) {
        ...
        const pageRect = currentPage.element.getBoundingClientRect();
        const containerRect = container.getBoundingClientRect();
        
        const scrollLeft = currentPage.element.offsetLeft - (containerRect.width - pageRect.width) / 2;
        const scrollTop = currentPage.element.offsetTop - (containerRect.height - pageRect.height) / 2;
...
async function renderPageToCanvas(page) {
    ...
    canvas.width = page.canvas.offsetWidth;
    canvas.height = page.canvas.offsetHeight;
    
    // Заливаем белым фоном (для PDF это важно)
    ...
    const elements = Array.from(page.canvas.children)
        .sort((a, b) => {
            // Сортируем элементы по z-index для правильного порядка отрисовки
